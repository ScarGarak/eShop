...
public class CopyOfLogInGUI extends JFrame implements ActionListener, KeyListener, MouseListener {
...
//	register objekte
	...
	public CopyOfLogInGUI() throws IOException {
		super("eShop - LogIn");
...
//		int id = Integer.parseInt(strId);
		int id = 1; // dient als platzhalter
...
//		nur zum testen
		linksPan.add(links);
		frameHeader.setBackground(Color.BLUE);
...
//		resizePan();
		...
		JLabel backLab = new JLabel("<html><u>zurück</u></html>");
		JLabel wpwLab = new JLabel("Passwort wiederholen");
		JLabel regLab = new JLabel("<html><u>Registrieren</u></html>");
...
//			einblenden anderer Komponenten
		...
		if (comp.equals(regLab)) {
			registrierung();
//			aufbau des loginFensters
		}
		if (comp.equals(backLab)) {
			System.out.println(backLab);
			zeichneLogin();
		}
	}
...
//		}
	...
	public static void main(String[] args) throws Exception {
		SwingUtilities.invokeLater(new Runnable() {
            public void run() {
				try {
					JFrame frame = new CopyOfLogInGUI();
